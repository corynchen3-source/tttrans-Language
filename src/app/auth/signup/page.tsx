    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50/30">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">加入译世界，开启翻译之旅</p>
        </div>

        <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/20 p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="p-3 rounded-lg bg-red-50 text-red-600 text-sm text-center font-medium">{error}</div>
            )}

            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1.5">用户名</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-slate-50/50 text-sm
          ...
          <p className="text-center text-sm text-slate-400 mt-5">
            已有账号？{" "}
            <Link href="/auth/signin" className="text-sky-600 hover:text-sky-700 font-medium hover:underline">
              立即登录
            </Link>
          </p>
        </div>
